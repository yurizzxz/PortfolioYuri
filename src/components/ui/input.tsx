...
export function InputRoot({ error = false, ...props }: InputRootProps) {
  return (
    <div
      className="w-full px-4 py-2.5 rounded-lg text-base bg-[var(--input)] border border-[var(--border)] focus-within:border-zinc-600 data-[error=true]:border-red-500 flex items-center gap-2"
      {...props}
    />
  );
}
